...
export class FacturaFormComponent {
  ...
  // Método para obtener la sucursal basada en el usuario conectado
  getSucursal() {
    // Primero, obtenemos el usuario usando el idUsuario
    this.gService
      .get('usuario', this.currentUser.idUsuario) // Reemplaza 'usuario' por la ruta correcta en tu API
      .pipe(
        takeUntil(this.destroy$),
        switchMap((user: any) => {
          // Guardamos la información del usuario traído
          this.currentUser = user;

          // Almacena la información del usuario en la variable 'encargado'
          this.encargado = user;

          // Ahora, obtenemos la sucursal usando el SucursalId del usuario
          return this.gService.get('sucursal', user.SucursalId); // Reemplaza 'sucursal' por la ruta correcta en tu API
        })
      )
      .subscribe((data: any) => {
        this.sucursal = data;
        ...
        this.facturaForm
          .get('sucursal')
          ?.setValue(this.sucursal, { emitEvent: false });
      });
  }
    ...
    if (this.selectedProductos.length > 0) {
      this.selectedProductos.forEach((producto) => {
        ...
        if (!productoEnCarrito) {
          ...
          this.productosCarrito.push(producto); // Añadir el producto a la tabla
    ...
    if (this.selectedServicios.length > 0) {
      this.selectedServicios.forEach((servicio) => {
        ...
        if (!servicioEnCarrito) {
          ...
          this.serviciosCarrito.push(servicio); // Añadir el servicio a la tabla
  ...
  submitForm(): void {
    this.cartService.getItems().subscribe((cartItems) => {
      ...
      const facturaDetalles = cartItems.map((item) => ({
        ...
        precio: item.precio, // Asegúrate de que el precio sea un número decimal
        cantidad: item.cantidad,
        impuesto: parseFloat(item.impuesto.toFixed(2)), // Asegúrate de que el impuesto sea un número decimal
      }));
      ...
      const data = {
        ...
        estado: false,
        facturaDetalle: facturaDetalles, // Asegúrate de que los detalles se envíen aquí
      };
      console.log(
        '************************************************************'
      );
